...
This work draws on the realized volatility forecasting literature. The foundational framework of @andersen2003 established that modeling log-transformed realized volatility with linear time series methods produces reliable forecasts, an insight that directly shaped our preprocessing and modeling choices. @corsi2009 introduced the HAR-RV model, demonstrating that simple additive structures over daily, weekly, and monthly horizons can capture the long-memory behavior of realized volatility remarkably well without requiring fractional integration. Our use of ARMA-type models on log RV follows the same spirit of parsimony while allowing for richer lag interactions through the MA component.

The decision to test VIX as an exogenous predictor was motivated by @bekaert2014, who decomposed VIX into an uncertainty component and a variance risk premium, showing that each carries distinct predictive information for future volatility and stock returns. Their finding that VIX embeds forward-looking content beyond what realized measures alone provide aligns with our result that SARIMAX with VIX substantially outperforms the pure ARIMA specification. @andersen1998 further informed our interpretation of forecast quality, establishing that modest out-of-sample $R^2$ values are the norm rather than a failure in volatility forecasting.

All market data (GOOG prices, VIX index, trading volume) were obtained through the `yfinance` Python API.
