...
Of all the quantities worth modeling, volatility is arguably the most useful. Predicting whether a stock will go up or down is famously difficult; the efficient market hypothesis holds that prices already reflect available information, so returns behave roughly like a random walk [@fama1970]. Volatility tells a different story. It clusters: calm periods tend to follow calm periods, and turbulent ones tend to persist. This predictability has real economic value. A trader who can anticipate a spike in volatility can hedge a portfolio with options, whose prices are tied directly to expected volatility, or scale down positions before the storm hits. Conversely, when implied volatility overshoots what actually materializes, strategies like selling straddles can profit from the gap [@bollerslev1986]. Realized volatility (RV), computed from observed returns, gives the ground truth for how much a stock actually moved, and forecasting it well is central to option pricing, risk budgeting, and portfolio construction.
